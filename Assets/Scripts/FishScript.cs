...
public class FishScript : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (gameplayProfile != null)
        {
            ...
            Hp = Mathf.Max(1f, gameplayProfile.health);
            CoinFish = Mathf.Max(0f, gameplayProfile.rewardValue);
    ...
    public void TakeDamage(SpriteRenderer targetSprite, float damage, int bulletId, int activeGunLevel)
    {
        ...
        {
            float resistance = gameplayProfile != null
    ...
    private IEnumerator FollowLeaderRoutine()
    {
        ...
        float speedBias = 1f + Mathf.Sin(phase * 2.31f) *
            schoolSpeedVariation;
    ...
    private void FishSystem(int bulletId, int activeGunLevel)
    {
        ...
        {
            TriggerGimmickExplosion(bulletId, activeGunLevel);
        }
    ...
    )
    {
        ...
        if (delay <= 0f)
        {
            PlayMainBossFrontGunSkillNow(profile, bulletId);
            return;
        }
    ...
    {
        float fishReward = Mathf.Max(0f, CoinFish);
        FishDeathProfile profile = deathProfile;
        float reward;
    ...
    private void PlayDeathPresentation(float coinAmount, int bulletId)
    {
        ...
        if (playMainBossFrontGunSkill)
        {
            if (profile == null)
            {
                PlayMainBossFrontGunSkillNow(null, bulletId);
            }
            else
            {
                TryScheduleMainBossFrontGunSkill(
    ...
    )
    {
        ...
        float[] hpValues =
        {
            60, 80, 100, 120, 150, 180, 220, 260, 320, 380, 450, 540,
            700, 850, 1000, 1200, 1450, 1700, 2000, 2350, 2750, 3200,
            3700, 4300, 5000, 5800, 6700, 7800, 9000, 10400, 12000,
            15000, 18000, 22000, 27000, 33000,
            42000, 55000, 72000,
            95000, 125000, 165000, 215000,
            300000, 420000, 580000
        };

        float[] rewardValues =
        {
            100, 130, 160, 190, 235, 280, 340, 400, 490, 580, 680, 810,
            1050, 1260, 1480, 1760, 2100, 2450, 2880, 3370, 3930, 4550,
            5250, 6080, 7050, 8150, 9400, 10900, 12600, 14600, 16800,
            22000, 26000, 32000, 39000, 48000,
            62000, 81000, 106000,
            145000, 190000, 250000, 325000,
            460000, 650000, 900000
        };
